...
if (!$conexion){
	...
	$sick = limpiarDatos($_POST['sick']);
	$medicaments = limpiarDatos($_POST['medicaments']);
	$alergy = limpiarDatos($_POST['alergy']);
	...
	$statement = $conexion->prepare('SELECT * FROM paciente WHERE cd LIKE :cd');
      ...
      <div id="container-body">
	<div class="alert alert-warning">
  		<strong>Error</strong> Cedula ya registrada, contactese con el adminitrador en el siguiente link <a href="contact.php">Contacto</a>
			</div>' 
			</div>
	<?php	
		}else{
	$statement = $conexion->prepare('INSERT INTO paciente (id, name, lastname, cd, gender, day_of_birth, address, email, phone, sick, medicaments, alergy) VALUES (null, :name, :lastname, :cd, :gender, :day_of_birth, :address, :email, :phone, :sick, :medicaments, :alergy)');
  ...
  <strong>Exito!</strong> Usted a sido registrado con exito, en un momento le sera enviado la cita a su correo.
</div>' 
</div>
<?php
}
...
<?php
include ("views/newpacient-view.php");
